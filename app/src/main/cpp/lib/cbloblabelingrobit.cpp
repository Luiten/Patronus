//http://cafe.naver.com/opencv/43952 레이블링 소스코드
...
CBlobLabelingRobit::CBlobLabelingRobit(const Mat& Img, const unsigned int nThreshold, int width, int height)
...
void CBlobLabelingRobit::_labeling()
{
    ...
    for(int nY = 0,posY=0; nY < m_height; nY++,posY+=m_width)
    {
        for(int nX = 0; nX < m_width; nX++)
        {
			...
			area = _check_Four_Neighbor(StartX, StartY, EndX, EndY);
            if(area > m_nThreshold)// && abs(StartX - EndX) < width && abs(StartY - EndY) < height)
            {
    ...
    m_nBlobs=m_num;
    delete[] m_isChecked; m_isChecked=nullptr;
    delete[] Pt_visited; m_isChecked=nullptr;
}
